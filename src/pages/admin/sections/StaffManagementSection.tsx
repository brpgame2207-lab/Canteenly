...
export const StaffManagementSection = () => {
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 h-[calc(100vh-140px)] flex flex-col">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 shrink-0">
        ...
        <div className="flex gap-2">
          ...
          <button 
            ...
            className="bg-brand hover:bg-brand-light text-white px-5 py-2.5 rounded-xl text-sm font-bold transition-all shadow-lg flex items-center gap-2"
          >
      ...
      {/* Sub-tabs */}
      <div className="shrink-0 flex gap-1 bg-white/5 p-1 rounded-2xl w-fit border border-white/5">
        {[
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 pb-10">
        <AnimatePresence mode="wait">
                  ...
                  <tbody className="divide-y divide-white/5">
                    {filteredStaff.map(member => (
                      <tr key={member.id} className="group hover:bg-white/[0.02] transition-colors">
                        ...
                        <td className="px-6 py-4 text-neutral-300">{member.role}</td>
                        <td className="px-6 py-4 text-neutral-300">{member.shift.split(' ')[0]}</td>
                        <td className="px-6 py-4">
                          <span className={cn(
                        ...
                        <td className="px-6 py-4 text-right">
                          <div className="flex justify-end gap-2">
                            <button onClick={() => handleToggleStatus(member.id)} className={cn("p-2 rounded-lg border transition-all", member.isActive ? "bg-white/5 border-white/10 text-neutral-400 hover:text-yellow-500" : "bg-green-500/10 border-green-500/20 text-green-500")}>
                              {member.isActive ? <ShieldOff size={16} /> : <Shield size={16} />}
                            </button>
                            <button 
                              onClick={() => {
                                setEditingStaff(member);
                                setFormData({ name: member.name, role: member.role, phone: member.phone, email: member.email, shift: member.shift });
                                setIsModalOpen(true);
                              }}
                            ...
                            <button onClick={() => handleDeleteStaff(member.id)} className="p-2 bg-white/5 border border-white/10 text-neutral-400 hover:text-red-500 rounded-lg transition-all">
                              <Trash2 size={16} />
                            </button>
                          </div>
                        </td>
          ...
          {activeSubTab === 'shifts' && (
            <motion.div key="shifts" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {SHIFTS.map(shift => (
                <div key={shift} className="glass-card p-6 rounded-3xl border border-white/5">
                  <div className="flex justify-between items-center mb-6">
                    <div className="flex items-center gap-3">
                      <div className="p-2 rounded-xl bg-brand/10 text-brand"><Clock size={20} /></div>
                      <div>
                        <h3 className="text-lg font-bold text-white">{shift.split(' ')[0]}</h3>
                        <p className="text-xs text-neutral-500">{shift.split('(')[1].replace(')', '')}</p>
                      </div>
                    </div>
                    <span className="text-xs font-bold text-neutral-400">{staff.filter(s => s.shift === shift && s.isActive).length} Assigned</span>
                  </div>
                  <div className="space-y-3">
                    {staff.filter(s => s.shift === shift && s.isActive).map(member => (
                      <div key={member.id} className="flex items-center justify-between p-3 rounded-2xl bg-white/5 border border-white/5">
                        <div className="flex items-center gap-3">
                          <div className="w-8 h-8 rounded-full bg-brand/20 text-brand flex items-center justify-center text-[10px] font-bold">{member.name.charAt(0)}</div>
                          <div className="text-sm font-bold text-white">{member.name}</div>
                        </div>
          ...
          {activeSubTab === 'attendance' && (
            <motion.div key="attendance" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="glass-card rounded-3xl border border-white/5 overflow-hidden">
              <div className="p-6 border-b border-white/5 flex justify-between items-center">
                <h3 className="font-bold text-white flex items-center gap-2"><Calendar size={18} className="text-brand" /> Attendance Tracker</h3>
                <div className="text-xs text-neutral-400">Today</div>
              </div>
              <table className="w-full text-left text-sm">
                <thead>
                  <tr className="bg-white/5 text-neutral-400 text-[10px] font-bold uppercase border-b border-white/5">
                    <th className="px-6 py-3">Staff</th>
                    <th className="px-6 py-3">Log In</th>
          ...
          {activeSubTab === 'activity' && (
            <motion.div key="activity" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              <div className="glass-card p-6 rounded-3xl border border-white/5">
                <div className="flex items-center gap-4">
                  <div className="p-3 bg-green-500/10 text-green-500 rounded-2xl"><BadgeCheck size={24} /></div>
                  <div>
              ...
              <div className="glass-card p-6 rounded-3xl border border-white/5">
                <div className="flex items-center gap-4">
                  <div className="p-3 bg-brand/10 text-brand rounded-2xl"><ShoppingBag size={24} /></div>
                  <div>
                    <div className="text-2xl font-bold text-white">268</div>
                    <div className="text-xs text-neutral-500">Orders Handled</div>
                  </div>
                </div>
              </div>
              <div className="glass-card p-6 rounded-3xl border border-white/5 col-span-1 lg:col-span-2">
                <h3 className="font-bold text-white mb-4 flex items-center gap-2"><History size={18} className="text-brand" /> Recent</h3>
                <div className="flex gap-4 overflow-x-auto pb-2 custom-scrollbar">
                  {staff.filter(s => s.isActive).map(member => (
                    <div key={member.id} className="flex flex-col items-center gap-1 shrink-0">
                      <div className="w-10 h-10 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center text-brand font-bold">{member.name.charAt(0)}</div>
                      <div className="text-[9px] font-bold text-white">{member.name.split(' ')[0]}</div>
                    </div>
                  ))}
                </div>
              </div>
      ...
      <AnimatePresence>
        {isModalOpen && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setIsModalOpen(false)}>
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} onClick={e => e.stopPropagation()} className="glass-card p-8 rounded-3xl w-full max-w-lg relative overflow-hidden">
              <div className="absolute -top-20 -right-20 w-48 h-48 bg-brand/20 blur-[80px] rounded-full pointer-events-none" />
              <div className="flex justify-between items-center mb-8">
                <div>
                  <h2 className="text-2xl font-bold text-white font-display">{editingStaff ? 'Update Staff' : 'Add New Staff'}</h2>
                  <p className="text-neutral-400 text-sm mt-1">Profile details.</p>
                </div>
                <button onClick={() => setIsModalOpen(false)} className="h-10 w-10 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 flex items-center justify-center text-neutral-400 hover:text-white transition-all"><X size={20} /></button>
              </div>

              <form onSubmit={handleAddOrEdit} className="space-y-4">
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-neutral-400 uppercase tracking-wider ml-1">Full Name</label>
                  <input type="text" required value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} placeholder="e.g. John Doe" className="w-full bg-white/5 border border-white/10 rounded-xl py-3 px-4 text-sm text-white focus:outline-none focus:ring-2 focus:ring-brand/50" />
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    ...
                    <input type="tel" value={formData.phone} onChange={e => setFormData({...formData, phone: e.target.value})} placeholder="9876543210" className="w-full bg-white/5 border border-white/10 rounded-xl py-3 px-4 text-sm text-white focus:outline-none" />
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-xs font-bold text-neutral-400 uppercase tracking-wider ml-1">Email</label>
                    <input type="email" value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})} placeholder="john@canteen.com" className="w-full bg-white/5 border border-white/10 rounded-xl py-3 px-4 text-sm text-white focus:outline-none" />
                  </div>
                </div>
                <button type="submit" className="w-full bg-brand hover:bg-brand-light text-white py-3.5 rounded-xl font-bold shadow-lg transition-all mt-4">
